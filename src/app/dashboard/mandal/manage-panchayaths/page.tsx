...
type Panchayath = {
  id: number;
  name: string;
  users: { id: number; username: string; name: string; candidateId: number }[];
  _count: { users: number; booths: number }
};

export default function ManagePanchayathPage() {
  ...
  const fetchPanchayaths = async () => {
    setLoading(true);
    try {
      const res = await fetch('/api/dashboard/mandal/panchayaths');
      if (res.ok) {
  ...
  const openAdminModal = (p: Panchayath) => {
    setSelectedPanchayath(p);
    setShowModal(true);
  };

  const filtered = panchayaths.filter(p => p.name.toLowerCase().includes(searchTerm.toLowerCase()));

  return (
    <div className="space-y-10">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-6 px-4">
        <div className="flex flex-col gap-1">
          <h1 className="text-3xl font-black text-gray-900 tracking-tight uppercase">Manage Panchayath</h1>
      ...
      {loading ? (
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {filtered.map((p) => {
            const ourAdmin = p.users.find(u => u.candidateId === currentCandidateId);
            const otherAdmins = p.users.filter(u => u.candidateId !== currentCandidateId);
            const hasAdmin = !!ourAdmin;
            const isTakenByOthers = otherAdmins.length > 0 && !hasAdmin;
            ...
            return (
              <div key={p.id} className="group p-8 rounded-[2.5rem] bg-white border border-gray-100 hover:border-emerald-600/20 shadow-sm transition-all space-y-6">
                ...
                <div className="space-y-1">
                  ...
                  {hasAdmin && <p className="text-[10px] font-black text-gray-900 uppercase tracking-widest pt-1 opacity-70">Admin: {ourAdmin.name} ({ourAdmin.username})</p>}
                  {!hasAdmin && isTakenByOthers && (
                    <div className="pt-2 space-y-1">
                      <p className="text-[9px] font-bold text-rose-500 uppercase tracking-widest leading-tight">
                        Administrative usernames are unique system-wide. "{otherAdmins[0].username}" is already in use by another candidate.
                      </p>
                    </div>
                  )}
                </div>
      ...
      {showModal && selectedPanchayath && (
        <SubUserModal
          role="PANCHAYATH_ADMIN"
          initialUser={selectedPanchayath.users.find(u => u.candidateId === currentCandidateId)}
          panchayathId={selectedPanchayath.id}
          panchayaths={panchayaths}
          onSuccess={() => { setShowModal(false); fetchPanchayaths(); }}
