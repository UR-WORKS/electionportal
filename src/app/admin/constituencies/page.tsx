    ...
    <div className="space-y-10 animate-in fade-in slide-in-from-bottom-4 duration-500">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-6 px-2">
        <div className="space-y-1">
          <h2 className="text-[10px] font-bold text-slate-400 uppercase tracking-[0.3em]">CONSTITUENCY MAPPING</h2>
          <h1 className="text-3xl font-black text-slate-900 tracking-tighter uppercase">Locations</h1>
        </div>
        <button 
          onClick={openAdd} 
      ...
      <div className="rounded-[2.5rem] bg-white border border-slate-100 shadow-sm overflow-hidden">
        {loading ? (
          <div className="flex flex-col items-center justify-center py-32 text-slate-400 gap-4">
            ...
            <p className="text-[10px] font-black uppercase tracking-widest">Loading locations...</p>
          </div>
        ) : items.length === 0 ? (
          <div className="flex flex-col items-center justify-center py-32 text-slate-300">
            <span className="text-6xl mb-6 opacity-20">🗺️</span>
            <p className="text-[10px] font-black uppercase tracking-[0.2em]">No locations configured yet</p>
          </div>
        ) : (
            ...
            <table className="w-full text-left border-collapse">
              <thead>
                <tr className="bg-slate-50/50 border-b border-slate-100">
                  <th className="px-10 py-6 text-[10px] font-black text-slate-400 uppercase tracking-[0.2em] w-20">#</th>
                  <th className="px-10 py-6 text-[10px] font-black text-slate-400 uppercase tracking-[0.2em]">Constituency Name</th>
                ...
                {items.map((item, i) => (
                  <tr key={item.id} className="group hover:bg-slate-50/30 transition-all">
                    <td className="px-10 py-6">
                      <span className="text-[11px] font-mono font-bold text-slate-300 group-hover:text-emerald-300 transition-colors">
        ...
        <div className="fixed inset-0 bg-slate-900/40 backdrop-blur-md flex items-center justify-center z-50 p-6 animate-in fade-in duration-300">
          <div className="rounded-[3rem] bg-white p-10 w-full max-w-md shadow-2xl shadow-slate-900/20 border border-slate-100 relative overflow-hidden">
            ...
            <div className="relative space-y-8">
              <div className="space-y-1">
                <h3 className="text-[10px] font-bold text-slate-400 uppercase tracking-[0.2em]">LOCATION SETUP</h3>
                <h2 className="text-2xl font-black text-slate-900 uppercase tracking-tight">{modal === 'add' ? 'New Region' : 'Edit Location'}</h2>
              </div>
              <form onSubmit={handleSave} className="space-y-6">
                <div className="space-y-2">
                  <label className="block text-[10px] font-black text-slate-400 uppercase tracking-widest px-1">Constituency Name</label>
