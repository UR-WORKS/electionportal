...
type panchayath = { id: number; name: string; constituencyId: number; constituency: { id: number; name: string } };

export default function PanchayathsPage() {
  const [items, setItems] = useState<panchayath[]>([]);
  const [constituencies, setConstituencies] = useState<Constituency[]>([]);
  const [loading, setLoading] = useState(true);
  const [modal, setModal] = useState<'add' | 'edit' | null>(null);
  const [editing, setEditing] = useState<panchayath | null>(null);
  ...
  const fetchData = useCallback(async () => {
    setLoading(true);
    const [pRes, cRes] = await Promise.all([fetch('/api/admin/panchayaths'), fetch('/api/admin/constituencies')]);
  ...
  function openEdit(item: panchayath) { setForm({ name: item.name, constituencyId: String(item.constituencyId) }); setEditing(item); setError(''); setModal('edit'); }
  function closeModal() { setModal(null); }

  async function handleSave(e: React.FormEvent) {
    e.preventDefault(); setSaving(true); setError('');
    const url = editing ? `/api/admin/panchayaths/${editing.id}` : '/api/admin/panchayaths';
  ...
  async function handleDelete(id: number) {
    if (!confirm('Delete this panchayath?')) return;
    const res = await fetch(`/api/admin/panchayaths/${id}`, { method: 'DELETE' });
    ...
    <div className="space-y-10 animate-in fade-in slide-in-from-bottom-4 duration-500">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-6 px-2">
        <div className="space-y-1">
          <h2 className="text-[10px] font-bold text-slate-400 uppercase tracking-[0.3em]">ADMINISTRATIVE MAPPING</h2>
          <h1 className="text-3xl font-black text-slate-900 tracking-tighter uppercase">Panchayaths</h1>
        </div>
        <button 
          onClick={openAdd} 
        ...
        >
          <span className="text-xl leading-none">+</span> Add Panchayath
        </button>
      </div>
      ...
      <div className="rounded-[2.5rem] bg-white border border-slate-100 shadow-sm overflow-hidden">
        {loading ? (
          <div className="flex flex-col items-center justify-center py-32 text-slate-400 gap-4">
            ...
            <p className="text-[10px] font-black uppercase tracking-widest">Loading panchayaths...</p>
          </div>
        ) : items.length === 0 ? (
          <div className="flex flex-col items-center justify-center py-32 text-slate-300">
            <span className="text-6xl mb-6 opacity-20">🏘️</span>
            <p className="text-[10px] font-black uppercase tracking-[0.2em]">No panchayaths identified yet</p>
          </div>
        ) : (
            ...
            <table className="w-full text-left border-collapse">
              <thead>
                <tr className="bg-slate-50/50 border-b border-slate-100">
                  <th className="px-10 py-6 text-[10px] font-black text-slate-400 uppercase tracking-[0.2em]">Panchayath Name</th>
                  <th className="px-10 py-6 text-[10px] font-black text-slate-400 uppercase tracking-[0.2em]">Constituency</th>
              ...
              <tbody className="divide-y divide-slate-50">
                {items.map((item) => (
                  <tr key={item.id} className="group hover:bg-slate-50/30 transition-all">
                    ...
                    <td className="px-10 py-6">
                      <span className="inline-flex px-4 py-1.5 rounded-xl bg-slate-50 text-slate-500 font-bold text-[10px] font-black uppercase border border-slate-100 group-hover:border-emerald-100 group-hover:bg-emerald-50 group-hover:text-emerald-600 transition-all tracking-widest">
        ...
        <div className="fixed inset-0 bg-slate-900/40 backdrop-blur-md flex items-center justify-center z-50 p-6 animate-in fade-in duration-300">
          <div className="rounded-[3rem] bg-white p-10 w-full max-w-md shadow-2xl shadow-slate-900/20 border border-slate-100 relative overflow-hidden">
            ...
            <div className="relative space-y-8">
              <div className="space-y-1">
                <h3 className="text-[10px] font-bold text-slate-400 uppercase tracking-[0.2em]">PANCHAYATH SETUP</h3>
              ...
              <form onSubmit={handleSave} className="space-y-6">
                <div className="space-y-2">
                  <label className="block text-[10px] font-black text-slate-400 uppercase tracking-widest px-1">Panchayath Name</label>
                  <input required value={form.name} onChange={e => setForm(f => ({ ...f, name: e.target.value }))}
                    className="w-full rounded-2xl bg-slate-50 border border-slate-100 px-6 py-4 text-sm font-black text-slate-900 placeholder-slate-300 outline-none focus:ring-4 focus:ring-emerald-500/10 focus:border-emerald-500 transition-all" placeholder="e.g. Rampur Panchayath" />
                </div>
                <div className="space-y-2">
                  <label className="block text-[10px] font-black text-slate-400 uppercase tracking-widest px-1">Constituency</label>
